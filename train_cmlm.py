...
def train(epoch, model, dataloader, criterionM, criterionL, optimizer, scheduler):
    ...
    for i, (src, tgt, src_lens, tgt_lens) in tqdm(enumerate(dataloader, 0), desc='train', total=len(opensub_dataset)//opt.realbatch):
        decoder_output_probs, pred_lengths_probs = model(
            src, tgt=tgt, src_lengths=src_lens, tgt_lengths=tgt_lens, mask_all=(epoch > 20))
        ...
        if (i+1) % opt.logstep == 0:
            ...
            distinct_1_score, distinct_2_score = 0., 0.
            show_gen_seq(
                src[:2], out_seqs[:2], tgt_lens[:2], tgt[:2], vocab_bulider, global_train_step, mode='train')
...
def eval(epoch, model, dataloader, criterionM, criterionL):
    ...
    with torch.no_grad():
        for i, (src, tgt, src_lens, tgt_lens) in tqdm(enumerate(dataloader, 0), desc='eval', total=len(imsdb_dataset)//opt.realbatch):
            decoder_output_probs, pred_lengths_probs = model(
                src, tgt=None, src_lengths=src_lens, tgt_lengths=None, mask_iter=4)
            ...
            decoder_output_probs_T = decoder_output_probs.permute(0, 2, 1)
            # loss
            loss_sentence = criterionM(decoder_output_probs_T, tgt_gold)
            loss_length = criterionL(pred_lengths_probs, tgt_lens)
            pred_lengths = torch.argmax(pred_lengths_probs, dim=1)
            ...
            global_valid_step += 1
        # summary
        show_gen_seq(src[:2], output_seqs[:2], tgt_lens[:2], tgt_gold[:2], vocab_bulider, global_valid_step, mode='valid')
